...
1. **해상도의 결정적 영향**: 
   - 1280 해상도 이미지를 사용한, Yolov8s 모델이 mAP50 ~ 0.6 > 0.5 달성
   - 640 해상도 이미지를 사용한, Yolov8n 와 Yolov8s 모델들은 mAP50 = 0.45 ~ 0.55 구간에서 수렴

2. **모델 규모의 영향**:
   - 이미지가 동일한 640 해상도에서 YOLOv8s가 YOLOv8n보다 **약 10% 높은 정확도** 를 가진다.

3. **수렴 속도**:
   - 고해상도 모델은 Epochs ~ 10 이내에 이미 목표 성능(mAP50 > 0.5) 초과 달성
...
### 2. 연산 효율성
YoLov8s 모델은 Yolov8n 대비 약간의 연산량 증가만으로도 유의미한 정확도를 확보할 수 있으므로, 실시간성이 보장되는 한 Small급 이상의 모델 사용을 권장합니다.

### 3. 향후 개선 방향
- 현재 ID 기반 임시 팀 분류(홀짝)를 '유니폼 색상 히스토그램 분석' 기반으로 전환하여 시도할 수 있다.
  -> 'Ta'와 'Tb'의 오분류율 감소 예상함.
- 본 프로젝트에서는 전체 '데이터셋의 10% 만 학습'했으므로, 데이터셋 모두를 학습하여 추가 결과를 비교하는 과제가 가능하다.
  -> 특히, 더 많은 수의 1280 해상도 이미지를 사용하면, 상대적으로 작은 이미지 객체인 축구공(Ball)의 탐지에 대한 학습이 더 잘 될 것으로 예상함.
